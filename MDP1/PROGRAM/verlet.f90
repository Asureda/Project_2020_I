...
SUBROUTINE VELO_VERLET(r,v,F)
    ...
    DO i=index_matrix(taskid+1,1), index_matrix(taskid+1,2)
        ...
        r(i,3)=PBC2(r(i,3),L)
    END DO
    ! print*, 'taskid',taskid+1,'first',index_matrix(taskid+1,1),'last',index_matrix(taskid+1,2)
    ...
    DO i=index_matrix(taskid+1,1), index_matrix(taskid+1,2)
        v(i,:)=v(i,:)+5d-1*(F(i,:))*h
    END DO
    ! print*,'taskid',taskid,'particula',index_matrix(taskid+1,1),'Force',F(index_matrix(taskid+1,1),:)
   DO k=1,3
